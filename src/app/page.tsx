...
import { QrCode, BarChart3, Globe, Zap, Shield, Smartphone, Menu, X } from "lucide-react";
...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-cream">
      ...
      <nav className="sticky top-0 z-40 bg-cream/95 backdrop-blur border-b border-[#e7e0d8]">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link href="/auth" className="text-sm text-[#4a3f35] hover:text-text transition-colors hidden sm:inline">
              Log in
            </Link>
            <Link
              href="/register"
            ...
            <details className="md:hidden relative">
              <summary className="list-none p-2 -mr-2 rounded-xl hover:bg-[#f0ebe4] transition-colors cursor-pointer">
                <Menu size={20} className="text-text" />
              </summary>
              <div className="absolute right-0 top-full mt-2 w-48 bg-surface border border-border rounded-2xl shadow-xl p-2 space-y-1">
                <a href="#features" className="block px-4 py-2.5 text-sm text-text rounded-xl hover:bg-cream transition-colors">Features</a>
                <a href="#how-it-works" className="block px-4 py-2.5 text-sm text-text rounded-xl hover:bg-cream transition-colors">How it works</a>
                <a href="#pricing" className="block px-4 py-2.5 text-sm text-text rounded-xl hover:bg-cream transition-colors">Pricing</a>
                <hr className="my-1 border-border" />
                <Link href="/auth" className="block px-4 py-2.5 text-sm text-text rounded-xl hover:bg-cream transition-colors">Log in</Link>
              </div>
            </details>
          </div>
        </div>
      ...
      {/* Hero */}
      <section className="max-w-6xl mx-auto px-4 sm:px-6 py-16 sm:py-20 md:py-28 text-center">
        <div className="inline-flex items-center gap-2 bg-gold/10 text-gold text-sm font-medium px-3 py-1.5 rounded-full mb-6">
          <Zap size={14} />
          No app downloads required
        </div>
        <h1 className="font-heading text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold text-text leading-tight mb-6">
          Your menu,<br />
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center px-4 sm:px-0">
          <Link
            href="/register"
            className="bg-primary text-white font-semibold px-6 sm:px-8 py-3.5 sm:py-4 rounded-xl text-base sm:text-lg hover:bg-[#c44a2a] transition-colors shadow-lg shadow-primary/20"
          >
            Start free — 14 days trial
          </Link>
          <a
            href="#how-it-works"
            className="border-2 border-secondary text-secondary font-semibold px-6 sm:px-8 py-3.5 sm:py-4 rounded-xl text-base sm:text-lg hover:bg-secondary/10 transition-colors"
          >
            See how it works
          </a>
        </div>
        <p className="text-sm text-muted mt-3 sm:mt-4">No credit card required</p>

        {/* Hero mockup */}
        <div className="mt-12 sm:mt-16 mx-0 sm:mx-4">
          <div className="bg-[#f5f1eb] rounded-2xl sm:rounded-3xl border border-[#e7e0d8] p-4 sm:p-6 md:p-8 max-w-3xl mx-auto shadow-xl">
            <div className="flex flex-col sm:flex-row items-center gap-6 sm:gap-8 justify-center">
              {/* Phone mockup */}
              <div className="bg-text rounded-2xl sm:rounded-3xl p-2 sm:p-3 w-40 sm:w-44 shadow-2xl">
                <div className="bg-cream rounded-xl sm:rounded-2xl overflow-hidden">
                  <div className="bg-secondary px-3 py-3 sm:py-4 text-white">
                    <div className="text-[10px] sm:text-xs font-medium opacity-80">The Grand Table</div>
                    <div className="text-xs sm:text-sm font-bold">Dinner Menu</div>
                  </div>
                  <div className="p-2 sm:p-3 space-y-1.5 sm:space-y-2">
                    <div className="text-[9px] sm:text-[10px] font-semibold text-muted uppercase tracking-wide">Starters</div>
                    {["Bruschetta — $12", "Soup du jour — $9", "Burrata — $16"].map((item) => (
                      <div key={item} className="text-[9px] sm:text-[10px] text-[#4a3f35] py-0.5 sm:py-1 border-b border-[#e7e0d8]">{item}</div>
                    ))}
                    <div className="text-[9px] sm:text-[10px] font-semibold text-muted uppercase tracking-wide pt-1 sm:pt-1">Mains</div>
              ...
              <div className="flex flex-col items-center gap-3">
                <div className="w-24 h-24 sm:w-32 sm:h-32 bg-text rounded-xl flex items-center justify-center">
                  <QrCode size={56} className="text-cream sm:hidden" />
                  <QrCode size={80} className="text-cream hidden sm:block" />
                </div>
      ...
      {/* How it works */}
      <section id="how-it-works" className="bg-[#f5f1eb] py-16 sm:py-20">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <h2 className="font-heading text-3xl sm:text-4xl font-bold text-text text-center mb-3">
            Up and running in minutes
          </h2>
          <p className="text-muted text-center mb-10 sm:mb-14 text-base sm:text-lg px-4">Three steps to your digital menu</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8">
            {[
              ...
              <div key={step} className="text-center px-2">
                <div className={`w-10 h-10 sm:w-12 sm:h-12 ${bg} text-white rounded-full flex items-center justify-center text-lg sm:text-xl font-bold mx-auto mb-4 font-heading`}>
                  {step}
                </div>
      ...
      <section id="features" className="py-16 sm:py-20 bg-cream">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          ...
          <p className="text-muted text-center mb-10 sm:mb-14 text-base sm:text-lg px-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {[
              ...
              <div key={title} className="p-5 sm:p-6 rounded-2xl border border-[#e7e0d8] hover:border-gold/50 hover:shadow-md transition-all bg-white/40">
                <div className={`w-9 h-9 sm:w-10 sm:h-10 ${iconBg} rounded-xl flex items-center justify-center mb-3 sm:mb-4`}>
                  {icon}
                </div>
                <h3 className="text-base sm:text-lg font-semibold text-text mb-1 sm:mb-2">{title}</h3>
      ...
      {/* Pricing */}
      <section id="pricing" className="bg-[#f5f1eb] py-16 sm:py-20">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <h2 className="font-heading text-3xl sm:text-4xl font-bold text-text text-center mb-3">
            Simple, transparent pricing
          </h2>
          <p className="text-muted text-center mb-10 sm:mb-14 text-base sm:text-lg px-4">Start free for 14 days. No credit card required.</p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-lg md:max-w-5xl mx-auto">
            {[
              {
                name: "Starter",
                price: "$19",
              ...
              <div
                ...
                className={`rounded-2xl p-6 sm:p-8 ${
                  highlight
                    ? "bg-secondary shadow-xl sm:shadow-2xl sm:scale-105"
              ...
              >
                <div className="mb-6">
                  ...
                  <div className={`text-3xl sm:text-4xl font-bold mb-1 font-heading ${highlight ? "text-white" : "text-text"}`}>
                    {price}<span className={`text-base sm:text-lg font-normal ${highlight ? "text-white/70" : "text-muted"}`}>/mo</span>
                  </div>
      ...
      <section className="bg-secondary py-16 sm:py-20">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 text-center">
          ...
          <p className="text-lg sm:text-xl text-cream/70 mb-6 sm:mb-8 px-2">
            Join hundreds of restaurants already using Plate. Set up your first menu in under 5 minutes.
          </p>
          <Link
            href="/register"
            className="inline-block bg-gold text-white font-semibold px-8 sm:px-10 py-3.5 sm:py-4 rounded-xl text-base sm:text-lg hover:bg-[#a67c30] transition-colors shadow-lg shadow-black/20"
          >
